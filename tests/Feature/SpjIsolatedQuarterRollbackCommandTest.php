...
class SpjIsolatedQuarterRollbackCommandTest extends TestCase
{
    use RefreshDatabase;
    ...
    public function test_quarter_rollback_smoke_resets_scope_and_preserves_baseline(): void
    {
        $school = $this->buildFixture();
        $hashBefore = hash_file('sha256', $this->baselinePath);

        $this->artisan('spj:test-quarter-rollback-copy', [
            'npsn' => $school->npsn,
            ...
            ->expectsOutputToContain('ISOLATED QUARTER ROLLBACK RESULT: PASS')
            ->expectsOutputToContain('Initial sequences   : 1,2,3')
            ->expectsOutputToContain('Sequence after      : 0')
            ->expectsOutputToContain('Renumbered sequence : 1')
            ->assertSuccessful();
        ...
        $activeSequences = DB::connection('school')->table('spj_documents')
            ...
            ->all();
        $this->assertSame([1], $activeSequences);
        $this->assertSame(1, (int) DB::connection('school')->table('document_number_sequences')->where('format_name', 'SPJ')->value('last_number'));
        $this->assertSame(2, DB::connection('school')->table('spj_packages')->where('status', 'DRAFT')->count());
    }

    public function test_quarter_rollback_smoke_rejects_baseline_path(): void
    {
        $school = $this->buildFixture();
        $hashBefore = hash_file('sha256', $this->baselinePath);
    ...
    private function buildFixture(): School
    {
        $school = School::create([
            'npsn' => '10208185',
            'school_code' => 'SDN.320',
            'name' => 'SDN Quarter Rollback',
        ]);

        $directory = storage_path('framework/testing');
        File::ensureDirectoryExists($directory);
        $this->baselinePath = $directory.'/spj-quarter-baseline-'.uniqid().'.sqlite';
        $this->copyPath = $directory.'/spj-quarter-copy-'.uniqid().'.sqlite';
    ...
    private function createReadyPackage(int $fiscalYearId, string $noBukti, string $sourceItemId): void
    {
        $transaction = Transaction::query()->create([
            ...
            'description' => 'Fixture quarter rollback',
            'payment_description' => 'Fixture quarter rollback',
