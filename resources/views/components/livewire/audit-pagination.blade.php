...
<div class="flex flex-col gap-3 border-t border-[var(--ui-line)] px-5 py-3 text-sm sm:flex-row sm:items-center sm:justify-between">
    <div class="flex items-center gap-3 text-[var(--ui-fg-muted)]">
        <span>Menampilkan {{ $paginator->firstItem() ?: 0 }}–{{ $paginator->lastItem() ?: 0 }} dari {{ number_format($paginator->total(), 0, ',', '.') }} {{ $noun }}</span>
