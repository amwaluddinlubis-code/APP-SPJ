...
<nav class="app-global-breadcrumb module-breadcrumb mb-4 flex min-w-0 flex-wrap items-center gap-1.5 text-sm" aria-label="Breadcrumb">
    @if(request()->routeIs('dashboard'))
        <span class="inline-flex items-center gap-2 rounded-full bg-slate-100 px-3 py-1.5 font-semibold text-slate-700" aria-current="page">
    ...
    @else
        ...
        @foreach($items as $item)
            ...
            @elseif($loop->last)
                <span class="max-w-full truncate rounded-lg bg-slate-100 px-2.5 py-1.5 font-semibold text-slate-800" aria-current="page">{{ $item['label'] }}</span>
            @else
