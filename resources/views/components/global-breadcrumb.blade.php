...
<nav class="module-breadcrumb mb-4 flex flex-wrap items-center gap-2 text-sm" aria-label="Breadcrumb">
    @if(request()->routeIs('dashboard'))
        <span class="inline-flex items-center gap-1.5 font-semibold text-slate-700" aria-current="page">
            <svg class="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" aria-hidden="true"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 12l2-2m0 0l7-7 7 7M5 10v10a1 1 0 001 1h3m10-11l2 2m-2-2v10a1 1 0 01-1 1h-3m-6 0a1 1 0 001-1v-4a1 1 0 011-1h2a1 1 0 011 1v4a1 1 0 001 1m-6 0h6" /></svg>
            Dashboard
        </span>
    @else
        <a href="{{ route('dashboard') }}" class="inline-flex items-center gap-1.5 font-medium text-slate-500 transition hover:text-slate-800">
            <svg class="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" aria-hidden="true"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 12l2-2m0 0l7-7 7 7M5 10v10a1 1 0 001 1h3m10-11l2 2m-2-2v10a1 1 0 01-1 1h-3m-6 0a1 1 0 001-1v-4a1 1 0 011-1h2a1 1 0 011 1v4a1 1 0 001 1m-6 0h6" /></svg>
            Dashboard
        </a>

        @foreach($items as $item)
            <svg class="h-4 w-4 shrink-0 text-slate-400" fill="none" viewBox="0 0 24 24" stroke="currentColor" aria-hidden="true"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5l7 7-7 7" /></svg>
            @if(!empty($item['url']))
                <a href="{{ $item['url'] }}" class="font-medium text-slate-500 transition hover:text-slate-800">{{ $item['label'] }}</a>
            @else
                <span class="font-semibold text-slate-700" aria-current="page">{{ $item['label'] }}</span>
            @endif
        @endforeach
    @endif
</nav>
