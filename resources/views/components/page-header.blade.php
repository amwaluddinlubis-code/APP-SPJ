...
<section class="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm">
    <div class="relative overflow-hidden {{ $gradient === 'theme' ? 'theme-header' : 'bg-gradient-to-br '.$gradient }} px-5 py-7 text-white sm:px-7 lg:py-8">
        ...
        <div class="relative mt-2 flex items-start gap-3">
            @if($icon)<span class="hidden h-9 w-9 place-items-center rounded-lg bg-white/15 text-white ring-1 ring-white/20 sm:grid">{{ $icon }}</span>@endif
            <div class="min-w-0">
    ...
    @if(isset($slot) && trim($slot) !== '')<div class="border-t border-slate-100 bg-white">{{ $slot }}</div>@endif
</section>
