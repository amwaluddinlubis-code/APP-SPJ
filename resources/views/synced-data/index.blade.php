                ...
                <section class="rounded-2xl border border-[var(--ui-line)] bg-[var(--ui-surface-base)] p-5 shadow sm:p-6">
                    ...
                    <div class="grid gap-3 sm:grid-cols-2 xl:grid-cols-3">
                        @foreach($items as $key => $item)
                            <a href="{{ route('synced-data.show', $key) }}" class="group rounded-xl border border-[var(--ui-line)] bg-[var(--ui-surface-base)] p-4 transition hover:-translate-y-0.5 hover:border-indigo-300 hover:shadow-md">
            ...
            <section class="overflow-hidden rounded-2xl border border-[var(--ui-line)] bg-[var(--ui-surface-base)] shadow">
                ...
                <div class="overflow-x-auto">
                    <table data-pagination="server" class="min-w-full divide-y divide-[var(--ui-line)] text-base">
                        <thead class="bg-[var(--ui-surface-soft)]"><tr><th class="sticky left-0 z-10 w-14 bg-[var(--ui-surface-soft)] px-4 py-3 text-center text-xs font-bold uppercase tracking-wide text-slate-500">No</th>
                        ...
                        <tbody class="divide-y divide-[var(--ui-line)] bg-[var(--ui-surface-base)]">
                            @forelse($rows as $index => $row)
                                <tr class="transition hover:bg-indigo-50/50"><td class="sticky left-0 z-10 bg-[var(--ui-surface-base)] px-4 py-3 text-center text-xs font-semibold text-slate-400">{{ $rows->firstItem() + $index }}</td>
                                    @foreach($table['select'] as $column)
                                        @php($parts = preg_split('/\s+as\s+/i', $column))
                                        @php($label = strtoupper(end($parts)))
                                        ...
                                        <td class="max-w-xs px-4 py-3 align-top text-slate-700">
                                            ...
                                            @elseif(in_array($label, $booleanColumns, true))<span class="rounded-full px-2 py-1 text-xs font-semibold {{ (bool) $value ? 'bg-emerald-50 text-emerald-700' : 'bg-[var(--ui-surface-muted)] text-slate-500' }}">{{ (bool) $value ? 'Ya' : 'Tidak' }}</span>
