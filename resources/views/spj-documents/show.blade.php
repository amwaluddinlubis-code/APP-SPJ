<x-layouts.tailwind-app>
    ...
    <div class="space-y-5">
        ...
        <div class="flex flex-wrap items-center justify-between gap-2">
            <a href="{{ route('spj.index', ['tab' => 'persiapan']) }}" class="rounded-md border border-[var(--ui-line-strong)] bg-[var(--ui-surface-base)] px-3 py-1.5 text-base font-semibold text-slate-700 shadow hover:bg-slate-50">← Semua paket</a>
        ...
        {{-- Header Paket --}}
        <section class="overflow-hidden rounded-xl border border-[var(--ui-line)] bg-[var(--ui-surface-base)] shadow">
        ...
        {{-- Validasi --}}
        <section class="overflow-hidden rounded-xl border {{ $validationIssues ? 'border-amber-200' : 'border-emerald-200' }} bg-[var(--ui-surface-base)] shadow">
        ...
        <section class="overflow-hidden rounded-xl border border-[var(--ui-line)] bg-[var(--ui-surface-base)] shadow" id="spj-tabs">
            {{-- Tab Header --}}
                ...
                <form id="spj-manual-form" method="POST" action="{{ route('spj.update', $package->id) }}" class="space-y-4 p-4" @submit="saving=true">@csrf @method('PUT')
                    <div class="rounded-lg border border-amber-200 bg-amber-50/60 p-3">
                        <div class="grid gap-3 sm:grid-cols-2">
                            ...
                            <div>
                                ...
                                <input name="spj_category" value="{{ old('spj_category', $transaction->spj_category ?: $transaction->spj_category ?: '') }}" class="mt-1 w-full rounded-md border border-amber-300 bg-[var(--ui-surface-base)] px-2.5 py-1.5 text-base placeholder:text-slate-400 focus:border-amber-400 focus:ring-1 focus:ring-amber-200" placeholder="ATK / konsumsi / honor">
                            </div>
                        </div>
                    </div>

                    <div class="grid gap-3 sm:grid-cols-2">
                        <div><label class="text-xs font-semibold text-slate-700">Uraian Dokumen</label><textarea name="description" rows="2" class="mt-1 w-full rounded-md border border-[var(--ui-line-strong)] bg-[var(--ui-surface-base)] px-3 py-2 text-base focus:border-indigo-300 focus:ring-1 focus:ring-indigo-200">{{ $transaction->document_description }}</textarea></div>
                        <div><label class="text-xs font-semibold text-slate-700">Untuk Pembayaran</label><textarea name="payment_description" rows="2" class="mt-1 w-full rounded-md border border-[var(--ui-line-strong)] bg-[var(--ui-surface-base)] px-3 py-2 text-base focus:border-indigo-300 focus:ring-1 focus:ring-indigo-200">{{ $transaction->payment_description }}</textarea></div>
                    </div>

                    <div><label class="text-xs font-semibold text-slate-700">Referensi Bayar</label><input name="payment_reference" value="{{ $transaction->payment_reference }}" class="mt-1 w-full rounded-md border border-[var(--ui-line-strong)] bg-[var(--ui-surface-base)] px-2.5 py-1.5 text-base placeholder:text-slate-400 focus:border-indigo-300 focus:ring-1 focus:ring-indigo-200" placeholder="No. transfer / pesanan"></div>
                    ...
                    <div data-spj-section="BARANG BELANJA_MODAL" class="rounded-lg border border-sky-200 bg-sky-50/60 p-3">
                        ...
                        <div class="mt-2 grid gap-3 sm:grid-cols-3">
                            <div><label class="text-xs font-medium text-sky-900">No. Invoice</label><input name="invoice_number" value="{{ $transaction->invoice_number }}" class="mt-1 w-full rounded-md border border-sky-200 bg-[var(--ui-surface-base)] px-2.5 py-1.5 text-base"></div>
                            <div><label class="text-xs font-medium text-sky-900">Tgl Invoice</label><input type="date" name="invoice_date" value="{{ $transaction->invoice_date?->format('Y-m-d') }}" class="mt-1 w-full rounded-md border border-sky-200 bg-[var(--ui-surface-base)] px-2.5 py-1.5 text-base"></div>
                            <div><label class="text-xs font-medium text-sky-900">Status</label><input name="invoice_status" value="{{ $transaction->invoice_status }}" class="mt-1 w-full rounded-md border border-sky-200 bg-[var(--ui-surface-base)] px-2.5 py-1.5 text-base" placeholder="LUNAS"></div>
                        </div>
                    </div>
                    ...
                    <div data-spj-section="UPAH JASA_HONORARIUM PEMELIHARAAN" class="rounded-lg border border-sky-200 bg-sky-50/60 p-3">
                        ...
                        <div class="mt-2 grid gap-3 sm:grid-cols-2">
                            <div><label class="text-xs font-medium text-sky-900">Uraian Pekerjaan</label><textarea name="work_description" rows="2" class="mt-1 w-full rounded-md border border-sky-200 bg-[var(--ui-surface-base)] px-3 py-2 text-base">{{ $transaction->work_description }}</textarea></div>
                            <div><label class="text-xs font-medium text-sky-900">Lokasi</label><input name="work_location" value="{{ $transaction->work_location }}" class="mt-1 w-full rounded-md border border-sky-200 bg-[var(--ui-surface-base)] px-2.5 py-1.5 text-base"></div>
                            <div><label class="text-xs font-medium text-sky-900">Tgl Mulai</label><input type="date" name="work_started_at" value="{{ $transaction->work_started_at?->format('Y-m-d') }}" class="mt-1 w-full rounded-md border border-sky-200 bg-[var(--ui-surface-base)] px-2.5 py-1.5 text-base"></div>
                            <div><label class="text-xs font-medium text-sky-900">Tgl Selesai</label><input type="date" name="work_completed_at" value="{{ $transaction->work_completed_at?->format('Y-m-d') }}" class="mt-1 w-full rounded-md border border-sky-200 bg-[var(--ui-surface-base)] px-2.5 py-1.5 text-base"></div>
                        </div>
                    </div>
                    ...
                    <div data-spj-section="PEMELIHARAAN" class="rounded-lg border border-sky-200 bg-sky-50/60 p-3">
                        ...
                        <div class="mt-2 grid gap-3 sm:grid-cols-2">
                            <div><label class="text-xs font-medium text-sky-900">No. SPK</label><input name="spk_number" value="{{ $transaction->spk_number }}" class="mt-1 w-full rounded-md border border-sky-200 bg-[var(--ui-surface-base)] px-2.5 py-1.5 text-base"></div>
                            <div><label class="text-xs font-medium text-sky-900">Tgl SPK</label><input type="date" name="spk_date" value="{{ $transaction->spk_date?->format('Y-m-d') }}" class="mt-1 w-full rounded-md border border-sky-200 bg-[var(--ui-surface-base)] px-2.5 py-1.5 text-base"></div>
                            <div><label class="text-xs font-medium text-sky-900">Nama Penandatangan</label><input name="signatory_name" value="{{ $transaction->signatory_name }}" class="mt-1 w-full rounded-md border border-sky-200 bg-[var(--ui-surface-base)] px-2.5 py-1.5 text-base"></div>
                            <div><label class="text-xs font-medium text-sky-900">Peran</label><input name="signatory_role" value="{{ $transaction->signatory_role }}" class="mt-1 w-full rounded-md border border-sky-200 bg-[var(--ui-surface-base)] px-2.5 py-1.5 text-base" placeholder="Pelaksana"></div>
                        </div>
                    </div>
                    ...
                    <div data-spj-section="UPAH JASA_HONORARIUM PEMELIHARAAN" class="overflow-hidden rounded-lg border border-amber-200 bg-amber-50/30">
                        ...
                        <div id="workers-desktop" class="hidden overflow-x-auto lg:block">
                            <table class="min-w-[760px] w-full text-base">
                                ...
                                <tbody class="divide-y divide-amber-100 bg-[var(--ui-surface-base)]">
                                    @foreach($workerRows as $index => $worker)
                                    <tr>
                                        <td class="px-2 py-1.5"><input name="workers[{{ $index }}][name]" value="{{ $worker?->name }}" class="w-full rounded-md border border-amber-200 bg-[var(--ui-surface-base)] px-2 py-1 text-base" placeholder="Nama"></td>
                                        <td class="px-2 py-1.5"><input name="workers[{{ $index }}][job_description]" value="{{ $worker?->job_description }}" class="w-full rounded-md border border-amber-200 bg-[var(--ui-surface-base)] px-2 py-1 text-base" placeholder="Tukang"></td>
                                        <td class="px-2 py-1.5"><input type="number" min="0" step=".5" name="workers[{{ $index }}][work_days]" value="{{ $worker?->work_days }}" class="w-20 rounded-md border border-amber-200 bg-[var(--ui-surface-base)] px-2 py-1 text-right text-base"></td>
                                        <td class="px-2 py-1.5"><input type="number" min="0" step="1" name="workers[{{ $index }}][daily_rate]" value="{{ $worker?->daily_rate }}" class="w-24 rounded-md border border-amber-200 bg-[var(--ui-surface-base)] px-2 py-1 text-right text-base"></td>
                                        ...
                                        <td class="px-2 py-1.5"><input name="workers[{{ $index }}][notes]" value="{{ $worker?->notes }}" class="w-full rounded-md border border-amber-200 bg-[var(--ui-surface-base)] px-2 py-1 text-base"></td>
                                    </tr>
                                    @endforeach
                                </tbody>
                            </table>
                        ...
                        <div id="workers-mobile" class="space-y-2 p-2 lg:hidden">
                            @foreach($workerRows as $index => $worker)
                            <div class="rounded-lg border border-amber-200 bg-[var(--ui-surface-base)] p-3 space-y-2">
                                <div class="grid grid-cols-2 gap-2">
                                    <div><label class="text-[11px] font-semibold text-amber-900">Nama</label><input name="workers[{{ $index }}][name]" value="{{ $worker?->name }}" class="mt-1 w-full rounded-md border border-amber-200 bg-[var(--ui-surface-base)] px-2.5 py-1.5 text-base"></div>
                                    <div><label class="text-[11px] font-semibold text-amber-900">Pekerjaan</label><input name="workers[{{ $index }}][job_description]" value="{{ $worker?->job_description }}" class="mt-1 w-full rounded-md border border-amber-200 bg-[var(--ui-surface-base)] px-2.5 py-1.5 text-base"></div>
                                </div>
                                <div class="grid grid-cols-2 gap-2">
                                    <div><label class="text-[11px] font-semibold text-amber-900">Hari</label><input type="number" min="0" step=".5" name="workers[{{ $index }}][work_days]" value="{{ $worker?->work_days }}" class="mt-1 w-full rounded-md border border-amber-200 bg-[var(--ui-surface-base)] px-2.5 py-1.5 text-base text-right"></div>
                                    <div><label class="text-[11px] font-semibold text-amber-900">Tarif/Hari</label><input type="number" min="0" step="1" name="workers[{{ $index }}][daily_rate]" value="{{ $worker?->daily_rate }}" class="mt-1 w-full rounded-md border border-amber-200 bg-[var(--ui-surface-base)] px-2.5 py-1.5 text-base text-right"></div>
                                </div>
                                <div class="flex items-center justify-between gap-2">
                                    ...
                                    <input name="workers[{{ $index }}][notes]" value="{{ $worker?->notes }}" class="flex-1 rounded-md border border-amber-200 bg-[var(--ui-surface-base)] px-2.5 py-1.5 text-base" placeholder="Catatan">
                                </div>
                            </div>
                            @endforeach
                        </div>
    ...
    <div id="template-preview-modal" class="fixed inset-0 z-50 hidden items-center justify-center bg-slate-950/60 p-3 backdrop-blur-sm" role="dialog" aria-modal="true" aria-labelledby="template-preview-title">
        <section class="flex h-[94vh] w-full max-w-[1500px] flex-col overflow-hidden rounded-xl bg-[var(--ui-surface-base)] shadow-2xl">
            <header class="flex shrink-0 flex-wrap items-center justify-between gap-3 border-b border-[var(--ui-line)] px-4 py-3"><div><p class="text-[11px] font-bold tracking-[.14em] text-sky-600">PRATINJAU TEMPLATE</p><h2 id="template-preview-title" class="text-sm font-bold text-slate-900">Dokumen SPJ</h2></div><div class="flex items-center gap-2"><button type="button" id="template-preview-print" class="rounded-md border border-[var(--ui-line-strong)] bg-[var(--ui-surface-base)] px-3 py-1.5 text-xs font-bold text-slate-700 hover:bg-slate-50">Cetak / PDF</button><button type="button" data-close-template-preview class="rounded-md bg-slate-800 px-3 py-1.5 text-xs font-bold text-white hover:bg-slate-950">Tutup</button></div></header>
            <div class="min-h-0 flex-1 bg-[var(--ui-surface-muted)] p-2"><iframe id="template-preview-frame" title="Pratinjau template SPJ" class="h-full w-full rounded-md border border-[var(--ui-line)] bg-[var(--ui-surface-base)]"></iframe></div>
        </section>
    </div>
