        ...
        <section id="modul-buat-spj" class="spj-builder order-2 overflow-hidden rounded-2xl border border-[var(--ui-line)] bg-[var(--ui-surface-base)] shadow-sm" x-data="{ category: '{{ $selectedSpjType }}', paymentMethod: @js($paymentMethod), isSiplah: @js($isSiplah) }">
            ...
            <div class="p-4 sm:p-5">
                ...
                <form method="POST" action="{{ route('spj.prepare', $transaction->id) }}" class="spj-builder-form rounded-xl border border-[var(--ui-line)] bg-[var(--ui-surface-base)] p-3 sm:p-4">
                    ...
                    @php($serviceRecipientRows = old('service_recipients', $transaction->serviceRecipients->map(fn ($recipient) => $recipient->only(['name', 'npwp', 'service_type', 'service_description', 'quantity', 'unit', 'rental_days', 'daily_rate', 'usage_started_at', 'usage_completed_at', 'receipt_number', 'payment_reference', 'agreement_number', 'agreement_date', 'is_receipt_recipient', 'notes']))->map(fn ($recipient) => [...$recipient, 'usage_started_at' => optional($recipient['usage_started_at'])->format('Y-m-d'), 'usage_completed_at' => optional($recipient['usage_completed_at'])->format('Y-m-d'), 'agreement_date' => optional($recipient['agreement_date'])->format('Y-m-d')])->values()->all() ?: [['name' => '', 'npwp' => '', 'service_type' => 'Jasa sewa harian', 'service_description' => '', 'quantity' => 1, 'unit' => 'unit', 'rental_days' => 1, 'daily_rate' => 0, 'usage_started_at' => $transactionDateLimit, 'usage_completed_at' => $transactionDateLimit, 'receipt_number' => '', 'payment_reference' => '', 'agreement_number' => '', 'agreement_date' => $transactionDateLimit, 'is_receipt_recipient' => false, 'notes' => '']]))
                    <section x-show="category === 'JASA_LAINNYA'" x-cloak x-data="{ rows: @js($serviceRecipientRows), total() { return this.rows.reduce((sum, row) => sum + (Number(row.quantity) || 0) * (Number(row.rental_days) || 0) * (Number(row.daily_rate) || 0), 0); }, add() { this.rows.push({name:'',npwp:'',service_type:'Jasa sewa harian',service_description:'',quantity:1,unit:'unit',rental_days:1,daily_rate:0,usage_started_at:@js($transactionDateLimit),usage_completed_at:@js($transactionDateLimit),receipt_number:'',payment_reference:'',agreement_number:'',agreement_date:@js($transactionDateLimit),is_receipt_recipient:false,notes:''}); }, money(value) { return new Intl.NumberFormat('id-ID', {style:'currency',currency:'IDR',maximumFractionDigits:0}).format(value); } }" class="mt-3 rounded-lg border border-[var(--ui-line)] bg-[var(--ui-surface-soft)] p-3">
                        <div class="flex flex-wrap items-start justify-between gap-3"><div><p class="text-[11px] font-bold uppercase tracking-wide text-[var(--theme-content-accent)]">Rincian jasa sewa harian</p><p class="mt-1 text-xs text-[var(--ui-fg-muted)]">Satu BPU dapat memiliki beberapa penerima. Setiap penerima mempunyai kuitansi dan referensi pembayarannya sendiri.</p></div><button type="button" @click="add()" class="ui-btn ui-btn-secondary px-3 py-1.5 text-xs font-bold">+ Penerima jasa</button></div>
                        <template x-for="(row, index) in rows" :key="index"><article class="mt-3 rounded-lg border border-[var(--ui-line)] bg-[var(--ui-surface-base)] p-3"><div class="mb-2 flex items-center justify-between"><b class="text-sm text-[var(--ui-fg-strong)]" x-text="'Penerima ' + (index + 1)"></b><button type="button" @click="rows.splice(index, 1)" class="ui-btn ui-btn-ghost px-2 py-1 text-xs text-rose-700">Hapus</button></div><div class="grid gap-2 md:grid-cols-4"><label class="md:col-span-2 text-[11px] font-semibold text-[var(--ui-fg-strong)]">Nama penerima <input required :name="`service_recipients[${index}][name]`" x-model="row.name" class="ui-input mt-1 text-sm"></label><label class="text-[11px] font-semibold text-[var(--ui-fg-strong)]">Jenis jasa/sewa <input required :name="`service_recipients[${index}][service_type]`" x-model="row.service_type" class="ui-input mt-1 text-sm"></label><label class="text-[11px] font-semibold text-[var(--ui-fg-strong)]">NPWP <input :name="`service_recipients[${index}][npwp]`" x-model="row.npwp" class="ui-input mt-1 text-sm"></label><label class="md:col-span-2 text-[11px] font-semibold text-[var(--ui-fg-strong)]">Objek/uraian jasa <input :name="`service_recipients[${index}][service_description]`" x-model="row.service_description" class="ui-input mt-1 text-sm" placeholder="Contoh: sewa sound system"></label><label class="text-[11px] font-semibold text-[var(--ui-fg-strong)]">Jumlah <input type="number" min="0" step=".01" :name="`service_recipients[${index}][quantity]`" x-model.number="row.quantity" class="ui-input mt-1 text-right text-sm"></label><label class="text-[11px] font-semibold text-[var(--ui-fg-strong)]">Satuan <input :name="`service_recipients[${index}][unit]`" x-model="row.unit" class="ui-input mt-1 text-sm"></label><label class="text-[11px] font-semibold text-[var(--ui-fg-strong)]">Hari <input type="number" min="0" step=".5" :name="`service_recipients[${index}][rental_days]`" x-model.number="row.rental_days" class="ui-input mt-1 text-right text-sm"></label><label class="text-[11px] font-semibold text-[var(--ui-fg-strong)]">Tarif/hari <input type="number" min="0" step="1" :name="`service_recipients[${index}][daily_rate]`" x-model.number="row.daily_rate" class="ui-input mt-1 text-right text-sm"></label><label class="text-[11px] font-semibold text-[var(--ui-fg-strong)]">Tgl mulai <input type="date" :name="`service_recipients[${index}][usage_started_at]`" x-model="row.usage_started_at" max="{{ $transactionDateLimit }}" class="ui-input mt-1 text-sm"></label><label class="text-[11px] font-semibold text-[var(--ui-fg-strong)]">Tgl selesai <input type="date" :name="`service_recipients[${index}][usage_completed_at]`" x-model="row.usage_completed_at" max="{{ $transactionDateLimit }}" class="ui-input mt-1 text-sm"></label><label class="text-[11px] font-semibold text-[var(--ui-fg-strong)]">No. kuitansi <input :name="`service_recipients[${index}][receipt_number]`" x-model="row.receipt_number" class="ui-input mt-1 text-sm"></label><label class="text-[11px] font-semibold text-[var(--ui-fg-strong)]">Referensi bayar <input :name="`service_recipients[${index}][payment_reference]`" x-model="row.payment_reference" class="ui-input mt-1 text-sm"></label><label class="text-[11px] font-semibold text-[var(--ui-fg-strong)]">No. PKS <input :name="`service_recipients[${index}][agreement_number]`" x-model="row.agreement_number" class="ui-input mt-1 text-sm"></label><label class="text-[11px] font-semibold text-[var(--ui-fg-strong)]">Tgl PKS <input type="date" :name="`service_recipients[${index}][agreement_date]`" x-model="row.agreement_date" max="{{ $transactionDateLimit }}" class="ui-input mt-1 text-sm"></label></div><label class="mt-2 inline-flex items-center gap-2 text-xs text-[var(--ui-fg)]"><input type="hidden" :name="`service_recipients[${index}][is_receipt_recipient]`" value="0"><input type="checkbox" :name="`service_recipients[${index}][is_receipt_recipient]`" value="1" x-model="row.is_receipt_recipient"> Penerima kuitansi utama</label></article></template>
                        <p class="mt-3 text-right text-sm font-bold text-[var(--ui-fg-strong)]">Total penerima jasa: <span x-text="money(total())"></span> <span class="ml-2 text-xs font-medium text-[var(--ui-fg-muted)]">(harus sama dengan nilai BKU)</span></p>
                    </section>
