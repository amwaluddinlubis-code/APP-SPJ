                ...
                <form method="POST" action="{{ route('spj.prepare', $transaction->id) }}" class="spj-builder-form rounded-xl border border-[var(--ui-line)] bg-[var(--ui-surface-base)] p-3 sm:p-4">
                    ...
                    <div class="mt-3 rounded-lg border border-[var(--ui-line)] bg-[var(--ui-surface-soft)] p-3">
                        ...
                        <div class="mt-2 grid gap-2 lg:grid-cols-4">
                            ...
                            <div>
                                ...
                                <input name="payment_reference" value="{{ $transaction->payment_reference }}" class="ui-input mt-1 text-sm" placeholder="No. cek/CMS/kuitansi">
                            </div>
                            <div>
                    ...
                    <div x-show="['BARANG','KONSUMSI'].includes(category)" x-cloak x-data="{ orderDate: @js($purchaseOrderDate), bapDate: @js($purchaseBapDate), bastDate: @js($purchaseBastDate), invoiceDate: @js($purchaseInvoiceDate) }" class="mt-3 rounded-lg border border-[var(--ui-line)] bg-[var(--ui-surface-soft)] p-3">
                        ...
                        <div class="mt-2 grid gap-2 md:grid-cols-3 xl:grid-cols-4">
                            <div><label class="text-[11px] font-semibold text-[var(--ui-fg-strong)]">No. Invoice/Faktur</label><input name="invoice_number" value="{{ $transaction->invoice_number }}" class="ui-input mt-1 text-sm" placeholder="No. invoice"></div>
                            <div><label class="text-[11px] font-semibold text-[var(--ui-fg-strong)]">Tgl Invoice</label><input type="date" name="invoice_date" x-model="invoiceDate" :min="paymentMethod === 'siplah' ? null : (bastDate || null)" max="{{ $transactionDateLimit }}" class="ui-input mt-1 text-sm"></div>
                            ...
                            <div><label class="text-[11px] font-semibold text-[var(--ui-fg-strong)]">No. Pesanan <span class="font-normal text-emerald-600 dark:text-emerald-400">(otomatis)</span></label><input readonly name="order_number" value="{{ $purchaseDetails?->order_number ?: $transaction->order_number }}" class="ui-input ui-input-readonly mt-1 text-sm"></div>
                            <div><label class="text-[11px] font-semibold text-[var(--ui-fg-strong)]">Tgl Pesanan</label><input type="date" name="order_date" x-model="orderDate" :max="bapDate || @js($transactionDateLimit)" class="ui-input mt-1 text-sm"></div>
                            ...
                            <div><label class="text-[11px] font-semibold text-[var(--ui-fg-strong)]">Tgl BAP</label><input type="date" name="bap_date" x-model="bapDate" :min="orderDate || null" :max="bastDate || @js($transactionDateLimit)" class="ui-input mt-1 text-sm"></div>
                            ...
                            <div><label class="text-[11px] font-semibold text-[var(--ui-fg-strong)]">Tgl BAST</label><input type="date" name="bast_date" x-model="bastDate" :min="bapDate || null" :max="invoiceDate || @js($transactionDateLimit)" class="ui-input mt-1 text-sm"></div>
                        </div>
                    </div>
                    ...
                    <div x-show="category === 'JASA_LAINNYA'" x-cloak class="mt-3 rounded-lg border border-[var(--ui-line)] bg-[var(--ui-surface-soft)] p-3">
                        <p class="text-[11px] font-bold uppercase tracking-wide text-[var(--theme-content-accent)]">Data jasa lainnya</p>
                        <div class="mt-2 grid gap-2 md:grid-cols-4">
                            <div class="md:col-span-2"><label class="text-[11px] font-semibold text-[var(--ui-fg-strong)]">Uraian Jasa</label><textarea name="work_description" rows="2" class="ui-textarea mt-1 px-2.5 py-1.5 text-sm" placeholder="Uraian jasa">{{ $transaction->work_description }}</textarea></div>
                            <div><label class="text-[11px] font-semibold text-[var(--ui-fg-strong)]">Lokasi/Unit Kerja</label><input name="work_location" value="{{ $transaction->work_location }}" class="ui-input mt-1 px-2.5 py-1.5 text-sm" placeholder="Lokasi/unit"></div>
                            <div><label class="text-[11px] font-semibold text-[var(--ui-fg-strong)]">Tanggal Mulai</label><input type="date" name="work_started_at" value="{{ $transaction->work_started_at?->format('Y-m-d') ?: $transactionDateLimit }}" max="{{ $transactionDateLimit }}" class="ui-input mt-1 px-2.5 py-1.5 text-sm"></div>
                            <div><label class="text-[11px] font-semibold text-[var(--ui-fg-strong)]">Tanggal Selesai</label><input type="date" name="work_completed_at" value="{{ $transaction->work_completed_at?->format('Y-m-d') ?: $transactionDateLimit }}" max="{{ $transactionDateLimit }}" class="ui-input mt-1 px-2.5 py-1.5 text-sm"></div>
                        </div>
                    </div>
