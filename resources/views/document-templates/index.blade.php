<x-layouts.tailwind-app>
    ...
    <div class="space-y-6">
        ...
        <x-ui.form-section title="Import Paket Template"
            description="Unggah satu workbook master XLSX yang berisi seluruh sheet canonical. Aplikasi memvalidasi paket terlebih dahulu lalu memisahkannya menjadi 11 template independen.">
            <form method="POST" action="{{ route('document-templates.store') }}" enctype="multipart/form-data"
                ...
                <div class="grid gap-4 lg:grid-cols-[minmax(0,1fr)_minmax(18rem,0.8fr)]">
                    <x-ui.field label="Workbook Master Template" for="template_package"
                        hint="XLSX maksimal 20 MB. Seluruh 11 sheet canonical wajib tersedia dan bebas ERROR."
                    ...
                    <div class="rounded-xl border border-slate-200 bg-amber-50/70 p-4">
                        <label class="flex items-start gap-3 text-sm font-semibold text-slate-700">
                            ...
                            <span>
                                ...
                                <span class="mt-1 block text-xs font-normal text-slate-500">Jika tidak dicentang dan
                                    template canonical XLSX sudah tersedia, import dibatalkan agar file lama tetap
                                    aman.</span>
                            </span>
                        </label>
                    </div>
                ...
                <div class="rounded-xl border border-slate-200 bg-white p-4">
                    ...
                    <p class="mt-3 text-xs text-slate-500">Sheet teknis <span class="font-mono">PLACEHOLDER_MAP</span>
                        tidak dibuat sebagai template aplikasi.</p>
                </div>

                <div class="ui-form-actions"><x-ui.button type="submit">Validasi & Import 11 Template</x-ui.button>
                </div>
            </form>
        ...
        <x-ui.form-section title="Tambah atau Ganti Template"
            description="Pilih jenis dokumen canonical, unggah file DOCX/XLSX, lalu tentukan kategori SPJ. File diperiksa terhadap kontrak placeholder sebelum disimpan.">
            <form method="POST" action="{{ route('document-templates.store') }}" enctype="multipart/form-data"
                class="space-y-5">@csrf
                <div class="grid gap-4 lg:grid-cols-2">
                    <x-ui.field label="Jenis Dokumen" for="document_type" :error="$errors->first('document_type')" required>
                        <x-ui.select id="document_type" name="document_type" required>
                            @foreach ($documentTypes as $value => $label)
                                <option value="{{ $value }}" @selected(old('document_type') === $value)>{{ $label }}
                                </option>
                            @endforeach
                        </x-ui.select>
                    </x-ui.field>
                    ...
                    <x-ui.field label="File Template" for="template_file"
                        hint="DOCX/XLSX maksimal 10 MB. ERROR akan menolak upload; WARNING tetap dapat disimpan."
                    ...
                    <fieldset class="rounded-xl border border-slate-200 bg-slate-50/70 p-4">
                        ...
                        <p class="mt-3 text-xs text-slate-500">Jika tidak ada kategori yang dipilih, template akan
                            tersedia untuk semua kategori SPJ.</p>
                    </fieldset>
                </div>
            ...
            @if (session('template_validation_warnings'))
                <div class="mt-5 rounded-xl border border-amber-200 bg-amber-50 p-4">
                    <p class="text-sm font-bold text-amber-800">Template berhasil disimpan dengan peringatan validasi.
                    </p>
                    <ul class="mt-2 space-y-1 text-xs text-amber-800">
        ...
        <section class="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm">
            <div class="border-b border-slate-100 px-5 py-4 sm:px-6">
                <div class="flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between">
                    <div>
                        ...
                        <p class="mt-1 text-sm text-slate-500">Pemeriksaan dilakukan pada template yang tampil di
                            halaman ini. ERROR perlu diperbaiki; WARNING tidak memblokir penggunaan.</p>
                    </div>
                    <div class="flex flex-wrap gap-2 text-xs font-bold">
                        <span class="rounded-full bg-emerald-100 px-3 py-1 text-emerald-800">VALID
                            {{ $validationValidCount }}</span>
                        <span class="rounded-full bg-amber-100 px-3 py-1 text-amber-800">WARNING
                            {{ $validationWarningCount }}</span>
                        <span class="rounded-full bg-rose-100 px-3 py-1 text-rose-800">ERROR
                            {{ $validationErrorCount }}</span>
                    </div>
                </div>
            </div>
            ...
            <div class="divide-y divide-slate-100">
                        ...
                        <summary
                            ...
                            <div>
                                <div class="flex flex-wrap items-center gap-2">
                                    ...
                                    <span
                                        class="font-mono text-[11px] text-violet-700">{{ $template->document_type }}</span>
                                    <span
                            ...
                            <span class="hidden text-xs font-semibold text-slate-500 group-open:inline">Tutup
                                rincian</span>
                        </summary>
                ...
                @empty
                    <div class="px-5 py-10 text-center text-sm text-slate-500">Belum ada template pada filter ini untuk
                        divalidasi.</div>
                @endforelse
            </div>
        </section>
                    ...
                    <x-ui.select id="category" name="category">
                        ...
                        @foreach ($categories as $category)
                            <option value="{{ $category }}" @selected(($filters['category'] ?? '') === $category)>
                                {{ $labels[$category] ?? str_replace('_', ' ', $category) }}</option>
                        @endforeach
                    </x-ui.select>
                </x-ui.field>
            ...
            <div class="overflow-x-auto">
                <table class="min-w-full text-sm" data-pagination="server">
                    ...
                    <tbody class="divide-y divide-slate-100">
                            ...
                            <tr class="odd:bg-white even:bg-slate-50/70 hover:bg-violet-50/60">
                                <td class="px-4 py-3 align-top">
                                    ...
                                    @if (!$isCanonical && $canonicalType)
                                        <p class="mt-1 text-[11px] text-amber-700">Alias lama untuk <span
                                                class="font-mono font-semibold">{{ $canonicalType }}</span>. Jika
                                            terjadi benturan dengan template canonical, data legacy dipertahankan dalam
                                            keadaan nonaktif.</p>
                                    @elseif(!$isCanonical)
                                        <p class="mt-1 text-[11px] text-slate-500">Belum memiliki padanan pada registry
                                            document type canonical v1.</p>
                                    @endif
                                </td>
                                <td class="px-4 py-3 align-top"><span
                                        class="rounded bg-slate-200 px-2 py-1 text-[11px] font-bold uppercase text-slate-700">{{ $template->format }}</span>
                                </td>
                                <td class="min-w-[320px] px-4 py-3 align-top">
                                    <form id="mapping-{{ $template->id }}" method="POST"
                                        action="{{ route('document-templates.mapping.update', $template->id) }}">@csrf
                                        @method('PUT')
                                ...
                                <td class="px-4 py-3 text-center align-top">
                                    <input form="mapping-{{ $template->id }}" type="hidden" name="is_active"
                                        value="0">
                                    <label
                                        class="inline-flex items-center gap-2 text-xs font-bold {{ $template->is_active ? 'text-emerald-700' : 'text-slate-500' }}">
                        ...
                        @empty
                            <tr>
                                <td colspan="5" class="px-5 py-12 text-center text-slate-500">Tidak ada template
