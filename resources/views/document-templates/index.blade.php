<x-layouts.tailwind-app>
    ...
    <div class="space-y-6">
        <x-page-header
            ...
            subtitle="Unggah, atur, dan tentukan kategori SPJ yang menggunakan setiap template dokumen."
        ...
        >
            ...
            <div class="grid divide-y divide-slate-100 sm:grid-cols-3 sm:divide-x sm:divide-y-0">
                ...
                <x-stat-item label="Kategori SPJ" :value="number_format(count($categories), 0, ',', '.')" hint="Kategori yang dapat dihubungkan ke template" value-class="text-emerald-700" />
                <x-stat-item label="Penanda Data" :value="number_format($placeholderCount, 0, ',', '.')" hint="Penanda yang dapat digunakan di template" value-class="text-slate-800" />
            </div>
        </x-page-header>
        ...
        <section class="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm">
            ...
            <form method="GET" class="grid gap-3 border-b border-slate-100 bg-slate-50/60 px-5 py-4 sm:grid-cols-[12rem_minmax(12rem,1fr)_auto_auto] sm:items-end">
                ...
                <x-ui.button type="submit">Tampilkan</x-ui.button><x-ui.button variant="secondary" :href="route('document-templates.index')">Hapus Filter</x-ui.button>
            </form>
            ...
            <div class="overflow-x-auto"><table class="min-w-full text-sm" data-pagination="server"><thead class="bg-slate-100"><tr class="text-left text-[11px] font-bold uppercase tracking-wide text-slate-600"><th class="px-4 py-3">Template</th><th class="px-4 py-3">Format</th><th class="px-4 py-3">Digunakan untuk</th><th class="px-4 py-3 text-center">Status</th><th class="px-4 py-3 text-right">Tindakan</th></tr></thead><tbody class="divide-y divide-slate-100">
                @forelse($templates as $template)
                    <tr class="odd:bg-white even:bg-slate-50/70 hover:bg-violet-50/60">
                        ...
                        <td class="whitespace-nowrap px-4 py-3 text-right align-top"><div class="flex justify-end gap-2"><x-ui.button form="mapping-{{ $template->id }}" type="submit">Simpan Perubahan</x-ui.button><form method="POST" action="{{ route('document-templates.destroy',$template->id) }}" data-confirm="Hapus template {{ $template->name }}? Template yang dihapus tidak dapat digunakan lagi. Lanjutkan?">@csrf @method('DELETE')<x-ui.button type="submit" variant="danger">Hapus</x-ui.button></form></div></td>
                    </tr>
                @empty<tr><td colspan="5" class="px-5 py-12 text-center text-slate-500">Tidak ada template yang sesuai dengan filter saat ini.</td></tr>@endforelse
            </tbody></table></div>
            @if($templates->hasPages())<div class="border-t border-slate-100 px-5 py-4">{{ $templates->links() }}</div>@endif
        </section>

        <details class="rounded-xl border border-slate-200 bg-white shadow-sm"><summary class="cursor-pointer px-5 py-4 text-sm font-bold text-slate-700">Lihat semua penanda data ({{ $placeholderCount }})</summary><div class="space-y-5 border-t border-slate-100 px-5 py-4"><p class="text-xs text-slate-500">Masukkan penanda ke template dengan kurung kurawal ganda, misalnya <code>&#123;&#123;NOMOR_SPJ&#125;&#125;</code>. Untuk rincian yang memiliki banyak baris, letakkan penanda rincian pada satu baris contoh. Aplikasi akan menggandakan baris tersebut sesuai jumlah data.</p>@foreach($placeholderGroups as $group => $markers)<section><h3 class="text-xs font-bold uppercase tracking-wide text-slate-600">{{ $group }}</h3><div class="mt-2 flex flex-wrap gap-1.5">@foreach($markers as $marker)<code class="rounded bg-slate-100 px-2 py-1 text-[11px] text-indigo-700">&#123;&#123;{{ $marker }}&#125;&#125;</code>@endforeach</div></section>@endforeach</div></details>
    </div>
</x-layouts.tailwind-app>
