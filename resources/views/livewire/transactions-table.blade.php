    ...
    <section class="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow">
        <x-ui.toolbar class="border-b border-[var(--ui-line)] px-5 py-4 sm:px-6">
            <div>
                ...
                <p class="mt-1 text-base" style="color: var(--ui-fg-muted)">Lengkapi uraian SPJ terlebih dahulu, lalu buka detail melalui kolom aksi.</p>
            </div>
            <x-slot:actions>
            ...
            @forelse($transactions as $transaction)
                <article class="rounded-xl border border-slate-200 bg-white p-4 shadow transition hover:shadow-md" wire:key="transaction-card-{{ $transaction->id }}">
                    <div class="flex items-start justify-between gap-2">
                        <div>
                            <p class="font-mono text-base font-bold" style="color: var(--theme-content-accent)">{{ $transaction->no_bukti }}</p>
                            <p class="mt-0.5 text-xs" style="color: var(--ui-fg-muted)">{{ $transaction->transaction_date?->translatedFormat('d F Y') ?? 'Tanggal belum tersedia' }}</p>
                        </div>
                    ...
                    <p class="mt-2 line-clamp-2 text-sm font-semibold" style="color: var(--ui-fg)">{{ $transaction->description ?: 'Tanpa uraian ARKAS' }}</p>
                    <p class="mt-1 line-clamp-2 text-sm" style="color: {{ filled($transaction->payment_description) ? 'var(--theme-content-accent)' : 'var(--ui-fg-muted)' }}">SPJ: {{ $transaction->payment_description ?: 'Deskripsi belanja belum diisi' }}</p>
                    <div class="mt-2 flex flex-wrap gap-1.5">
                        @if($transaction->spj_category)<x-ui.badge class="text-xs">{{ $spjTypeLabel($transaction->spj_category) }}</x-ui.badge>@endif
                        @if($this->paymentMethodFor($transaction) === 'siplah')<x-ui.badge class="text-xs">SiPLah</x-ui.badge>@endif
                        <x-ui.badge variant="neutral" class="text-xs">{{ $transaction->items_count }} item</x-ui.badge>
                    </div>
                    <p class="mt-2 truncate text-xs" style="color: var(--ui-fg-muted)">Penerima: {{ $transaction->effective_receipt_recipient_name ?: $transaction->recipient_name ?: 'Belum diisi' }}</p>

                    <div class="mt-3 flex items-center justify-between border-t pt-3">
                        <div>
                            <p class="text-xs" style="color: var(--ui-fg-muted)">Bruto · Pajak</p>
                            <p class="text-sm font-bold" style="color: var(--ui-fg)">{{ $rupiah($transaction->gross_amount) }} · {{ $rupiah($transaction->tax_total) }}</p>
                        </div>
                        <div class="flex items-center gap-2" aria-label="Aksi transaksi">
                            <button
                                type="button"
                                ...
                                title="Ubah data SPJ"
                                aria-label="Ubah data SPJ"
                                class="inline-flex h-9 w-9 items-center justify-center rounded-lg border border-slate-200 text-slate-600 transition hover:border-indigo-300 hover:bg-indigo-50 hover:text-indigo-700 disabled:cursor-not-allowed disabled:opacity-40"
                            ><x-ui-icon name="edit" class="h-4 w-4" /></button>

                            @if(filled($transaction->payment_description))
                                <a href="{{ route('transactions.show', $transaction) }}" wire:navigate title="Buka detail" aria-label="Buka detail" class="inline-flex h-9 w-9 items-center justify-center rounded-lg border border-slate-200 text-slate-600 transition hover:border-indigo-300 hover:bg-indigo-50 hover:text-indigo-700"><x-ui-icon name="document" class="h-4 w-4" /></a>
                            @else
                                <button type="button" x-on:click="showDescriptionWarning()" title="Buka detail — deskripsi belanja belum lengkap" aria-label="Buka detail belum tersedia" class="inline-flex h-9 w-9 items-center justify-center rounded-lg border border-slate-200 bg-slate-50 text-slate-300 transition hover:border-amber-300 hover:bg-amber-50 hover:text-amber-600"><x-ui-icon name="document" class="h-4 w-4" /></button>
                            @endif
                        </div>
                    </div>
                </article>
            ...
            <table data-pagination="server" class="min-w-full divide-y divide-slate-200 text-sm">
                <thead class="bg-slate-50">
                    <tr>
                        ...
                        <th class="px-4 py-3 text-left text-xs font-bold uppercase tracking-wide text-slate-500">Uraian & SPJ</th>
                        ...
                        <th class="px-5 py-3 text-center text-xs font-bold uppercase tracking-wide text-slate-500">Aksi</th>
                    </tr>
                </thead>
                <tbody class="divide-y divide-slate-100 bg-white">
                    @forelse($transactions as $transaction)
                        <tr class="transition hover:bg-indigo-50/50" wire:key="transaction-row-{{ $transaction->id }}">
                            <td class="px-5 py-4 align-top">
                                <p class="font-mono font-bold" style="color: var(--theme-content-accent)">{{ $transaction->no_bukti }}</p>
                                <p class="mt-1 text-xs" style="color: var(--ui-fg-muted)">{{ $transaction->transaction_date?->translatedFormat('d F Y') ?? 'Tanggal belum tersedia' }}</p>
                                <div class="mt-1.5 flex flex-wrap gap-1.5">
                                    @if($this->paymentMethodFor($transaction) === 'siplah')<x-ui.badge class="text-xs">SiPLah</x-ui.badge>@endif
                                    @if($transaction->spjPackage?->status === 'CANCELLED')
                                        <x-ui.status-badge status="CANCELLED" label="Dibatalkan" size="xs" />
                                    @elseif($transaction->spjPackage?->document_number)
                                        <x-ui.status-badge status="NUMBERED" label="Bernomor" size="xs" />
                                    @elseif($transaction->spjPackage)
                                        <x-ui.status-badge status="DRAFT" label="Draft paket" size="xs" />
                            ...
                            <td class="max-w-xl px-4 py-4 align-top">
                                <p class="line-clamp-2 font-semibold" style="color: var(--ui-fg)">{{ $transaction->description ?: 'Tanpa uraian ARKAS' }}</p>
                                <p class="mt-1 line-clamp-2 text-xs {{ filled($transaction->payment_description) ? 'font-semibold' : '' }}" style="color: {{ filled($transaction->payment_description) ? 'var(--theme-content-accent)' : 'var(--ui-fg-muted)' }}">SPJ: {{ $transaction->payment_description ?: 'Deskripsi belanja belum diisi' }}</p>
                                <div class="mt-2 flex flex-wrap gap-1.5">
                                    @if($transaction->spj_category)<x-ui.badge class="text-xs">{{ $spjTypeLabel($transaction->spj_category) }}</x-ui.badge>@endif
                                    <x-ui.badge variant="neutral" class="text-xs">{{ $transaction->items_count }} item</x-ui.badge>
                                    @if(strtoupper((string) $transaction->source_status) === 'SOURCE_MISSING')<x-ui.status-badge status="SOURCE_MISSING" label="Tidak muncul di sync" size="xs" />@endif
                                    @if($transaction->requires_reconciliation)<x-ui.status-badge status="REQUIRES_RECONCILIATION" label="Rekonsiliasi" size="xs" />@endif
                                </div>
                                <p class="mt-2 truncate text-xs" style="color: var(--ui-fg-muted)">Penerima: {{ $transaction->effective_receipt_recipient_name ?: $transaction->recipient_name ?: 'Belum diisi' }}</p>
                            </td>
                            <td class="max-w-xs px-4 py-4 align-top">
                                <p class="font-mono text-xs font-semibold" style="color: var(--theme-content-accent)">{{ $transaction->activity_code ?: '—' }}</p>
                                <p class="mt-1 truncate text-xs" style="color: var(--ui-fg-muted)">{{ $transaction->account_code ?: 'Rekening belum tersedia' }}</p>
                            </td>
                            <td class="whitespace-nowrap px-4 py-4 text-right align-top">
                                ...
                                <p class="mt-1 text-xs text-amber-700">Pajak {{ $rupiah($transaction->tax_total) }}</p>
                            </td>
                            <td class="px-5 py-4 align-top">
                    ...
                    @empty
                        <tr><td colspan="5" class="px-5 py-14 text-center"><p class="font-semibold" style="color: var(--ui-fg)">Transaksi belum ditemukan.</p><p class="mt-1 text-base" style="color: var(--ui-fg-muted)">Jalankan Sinkron Semua ARKAS atau ubah filter pencarian.</p></td></tr>
                    @endforelse
                </tbody>
            </table>
        </div>
    ...
    <div x-show="warningOpen" x-cloak class="fixed inset-0 z-[60] flex items-center justify-center bg-slate-950/60 p-4 backdrop-blur-sm" role="dialog" aria-modal="true" aria-labelledby="transaction-warning-title" x-on:click.self="closeWarning" x-on:keydown.escape.window="closeWarning">
        <div class="w-full max-w-md rounded-xl bg-white p-5 shadow-2xl">
            <div class="flex items-start gap-3">
                ...
                <div>
                    <p class="text-xs font-bold uppercase tracking-wide text-amber-700">Warning</p>
                    <h2 id="transaction-warning-title" class="mt-1 text-lg font-bold text-slate-900" x-text="warningMessage"></h2>
                    <p class="mt-2 text-sm text-slate-500">Gunakan ikon Ubah Data SPJ untuk mengisi deskripsi belanja sebelum membuka detail transaksi.</p>
                </div>
            </div>
            <div class="mt-5 flex justify-end">
                <button type="button" x-on:click="closeWarning" class="rounded-md bg-amber-600 px-4 py-2 text-sm font-bold text-white hover:bg-amber-700">Mengerti</button>
            </div>
        </div>
    </div>
