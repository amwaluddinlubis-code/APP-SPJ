        ...
        <div class="grid gap-3 p-4 lg:hidden">
            @forelse($transactions as $transaction)
                <article class="rounded-xl border border-slate-200 bg-white p-4 shadow transition hover:shadow-md" wire:key="transaction-card-{{ $transaction->id }}">
                    ...
                    <div class="mt-1 flex flex-wrap gap-1.5">@if($transaction->spj_category)<p class="text-xs font-semibold" style="color: var(--theme-content-accent)">SPJ: {{ $spjTypeLabel($transaction->spj_category) }}</p>@endif @if($this->paymentMethodFor($transaction) === 'siplah')<x-ui.badge class="text-xs">SiPLah</x-ui.badge>@endif</div>
