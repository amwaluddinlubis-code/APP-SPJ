<div data-livewire-tabs="true" class="db-tabbar">
    <nav class="flex gap-1 overflow-x-auto" aria-label="Navigasi database">
        ...
        <button type="button" wire:click="selectTab('list')" data-tab="list" class="tab-btn" data-active="{{ $activeTab === 'list' ? 'true' : 'false' }}">Database Sekolah <span class="db-tab-count">{{ $tableCount }}</span></button>
        <button type="button" wire:click="selectTab('central')" data-tab="central" class="tab-btn" data-active="{{ $activeTab === 'central' ? 'true' : 'false' }}">Database Pusat <span class="db-tab-count">{{ $centralTableCount }}</span></button>
