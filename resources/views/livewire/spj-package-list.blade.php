...
<div>
    ...
    <div class="flex flex-col gap-3 border-t border-[var(--ui-line)] bg-[var(--ui-surface-soft)] px-5 py-4 sm:flex-row sm:items-center sm:justify-between">
        <div class="flex items-center gap-2 text-xs" style="color: var(--ui-fg-muted)">
            ...
            <span>Menampilkan <span class="font-semibold" style="color: var(--ui-fg-strong)">{{ $packageList->firstItem() ?? 0 }}â€“{{ $packageList->lastItem() ?? 0 }}</span> dari <span class="font-semibold" style="color: var(--ui-fg-strong)">{{ number_format($packageList->total(), 0, ',', '.') }}</span> paket</span>
        </div>
