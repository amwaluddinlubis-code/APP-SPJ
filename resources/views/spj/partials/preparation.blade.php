<div x-show="tab === 'persiapan'" x-transition>
    <div class="border-b border-[var(--ui-line)] px-5 py-5 sm:px-6">
        <div class="flex flex-col gap-1 sm:flex-row sm:items-end sm:justify-between">
            <div>
                <h2 class="font-bold text-slate-900">Antrean persiapan SPJ</h2>
                <p class="mt-1 text-sm text-slate-500">Pilih transaksi, periksa rincian, lalu siapkan paket dokumennya.
                </p>
            </div>
            <p class="max-w-xl text-xs font-medium text-slate-400">Prioritas: draft perlu dilengkapi → belum disiapkan →
        ...
        <nav class="spj-work-queue mt-3 grid gap-2 sm:grid-cols-2 xl:grid-cols-5" aria-label="Pilih pekerjaan SPJ">
            ...
            <a href="{{ route('spj.index', array_merge($preparationQuery, ['state' => 'needs_details'])) }}"
                class="spj-work-queue-item"><span>Rincian belum
            ...
            <a href="{{ route('spj.index', array_merge($preparationQuery, ['state' => 'unprepared'])) }}"
                class="spj-work-queue-item"><span>Belum dibuat
                    paket</span><strong>{{ $workQueueCounts['unprepared'] ?? 0 }}</strong></a>
        ...
        <form method="GET" class="spj-filter-bar mt-3 rounded-xl border border-[var(--ui-line)] bg-slate-50/70 p-3">
            ...
            <div class="grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
                ...
                <x-ui.field label="Status" for="spj-preparation-state"><x-ui.select id="spj-preparation-state"
                        ...
                        <option value="needs_details" @selected(($filters['state'] ?? null) === 'needs_details')">Rincian belum ada</option>
                        <option value="ready" @selected(($filters['state'] ?? null) === 'ready')">Siap dibuat</option>
                        <option value="unprepared" @selected(($filters['state'] ?? null) === 'unprepared')">Belum dibuat paket</option>
    ...
    <div class="overflow-x-auto">
        <table class="min-w-full divide-y divide-[var(--ui-line)] text-base">
            <thead class="bg-[var(--ui-surface-soft)]">
                <tr>
                    <th class="px-5 py-3 text-left text-xs font-bold uppercase tracking-wide text-slate-500">Bukti /
                        Tanggal</th>
                    <th class="px-4 py-3 text-left text-xs font-bold uppercase tracking-wide text-slate-500">Status</th>
                    <th class="px-4 py-3 text-left text-xs font-bold uppercase tracking-wide text-slate-500">Uraian /
                        Penerima</th>
                    <th class="px-4 py-3 text-left text-xs font-bold uppercase tracking-wide text-slate-500">Kategori /
                        Rincian</th>
                    <th class="px-4 py-3 text-right text-xs font-bold uppercase tracking-wide text-slate-500">Nilai</th>
                    <th class="px-5 py-3 text-right text-xs font-bold uppercase tracking-wide text-slate-500">Tindakan
                    </th>
                </tr>
            </thead>
            <tbody class="divide-y divide-[var(--ui-line)] bg-[var(--ui-surface-base)]">
                @forelse($transactions ?? [] as $transaction)
                    <tr class="transition hover:bg-violet-50/50">
                        <td class="px-5 py-4">
                            <p class="font-mono font-bold text-violet-700">{{ $transaction->no_bukti }}</p>
                            <p class="mt-1 text-xs text-slate-500">
                                {{ $transaction->transaction_date?->translatedFormat('d F Y') }}</p>
                        </td>
                        <td class="px-4 py-4">
                        ...
                        <td class="max-w-sm px-4 py-4">
                            <p class="truncate font-semibold text-slate-800">
                                {{ $transaction->payment_description ?: $transaction->description ?: 'Tanpa uraian' }}
                            </p>
                            <p class="mt-1 truncate text-xs text-slate-500">
                                {{ $transaction->recipient_name ?: 'Penerima belum diisi' }}</p>
                        </td>
                        <td class="px-4 py-4">
                            <p class="text-xs font-bold text-indigo-700">
                                {{ $spjTypeLabel($transaction->spj_category) }}</p>
                            <p class="mt-1 text-xs text-slate-500">{{ $transaction->items_count }} rincian</p>
                        </td>
                        <td class="whitespace-nowrap px-4 py-4 text-right font-semibold text-slate-800">
                            {{ $rupiah($transaction->gross_amount) }}</td>
                        <td class="px-5 py-4 text-right">
                            ...
                            <x-ui.button :href="route('transactions.show', $transaction->id) . '#modul-buat-spj'">Lengkapi &amp; siapkan →</x-ui.button>@else<span
                                    class="text-xs text-slate-400">Rincian belum ada</span>
                            @endif
                        </td>
                    </tr>
                @empty<tr>
                        <td colspan="6" class="px-5 py-14 text-center">
                            <p class="font-semibold text-slate-700">Belum ada transaksi tersinkron.</p>
                            <p class="mt-1 text-base text-slate-500">Jalankan Sinkron Semua ARKAS terlebih dahulu.</p>
                        </td>
                    </tr>
                @endforelse
            </tbody>
    ...
    <div
        class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 border-t border-[var(--ui-line)] px-5 py-4 bg-slate-50/30">
