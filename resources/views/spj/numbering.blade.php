    ...
    <div class="spj-semantic-workspace space-y-6">
        ...
        >
            ...
            <div class="grid divide-y divide-[var(--ui-line)] sm:grid-cols-2 lg:grid-cols-4 sm:divide-x sm:divide-y-0">
                ...
                <x-stat-item label="Siap diberi nomor" :value="number_format($selectedSummary['ready'] ?? 0, 0, ',', '.')" hint="Paket sudah lengkap dan siap diproses" value-class="text-[var(--theme-content-accent)]" />
            ...
            <div class="grid gap-3 md:grid-cols-5">
                @foreach([
                    ...
                    <div class="rounded-xl border border-[var(--ui-line)] bg-[var(--ui-surface-soft)] p-4">
                        ...
                        <p class="mt-3 text-sm font-bold text-[var(--ui-fg-strong)]">{{ $title }}</p>
                        <p class="mt-1 text-xs leading-5 text-[var(--ui-fg-muted)]">{{ $description }}</p>
                    </div>
                @endforeach
            </div>
        </x-section-card>
        ...
        <x-section-card title="Periksa paket yang akan diberi nomor" description="Daftar ini hanya untuk pemeriksaan. Nomor belum dibuat sampai Anda menekan tombol penomoran.">
                    ...
                    <tbody class="divide-y divide-[var(--ui-line)] bg-[var(--ui-surface-base)]">
                        @forelse($previewPackages as $package)
                            <tr class="hover:bg-[var(--ui-table-row-hover)]">
                                <td class="px-4 py-3"><p class="font-mono font-bold text-[var(--theme-content-accent)]">{{ $package->transaction->no_bukti }}</p><p class="mt-1 text-xs text-[var(--ui-fg-muted)]">{{ $package->transaction->transaction_date?->translatedFormat('d F Y') }}</p></td>
                                <td class="max-w-md px-4 py-3"><p class="truncate font-semibold text-[var(--ui-fg-strong)]">{{ $package->transaction->payment_description ?: $package->transaction->description ?: 'Uraian belum tersedia' }}</p><p class="mt-1 truncate text-xs text-[var(--ui-fg-muted)]">{{ $package->transaction->recipient_name ?: 'Penerima belum diisi' }}</p></td>
            ...
            @if(auth()->user()->isAdministrator())
                <form method="POST" action="{{ route('spj.quarter-numbering') }}" class="mt-5 rounded-2xl border border-[var(--theme-accent-soft)] bg-[var(--theme-accent-soft)] p-4" x-data="{ reviewOpen: false }">
                    @csrf
                    ...
                    <div x-cloak x-show="reviewOpen" x-transition.opacity class="fixed inset-0 z-[80] flex items-center justify-center bg-slate-950/55 p-4" @keydown.escape.window="reviewOpen = false">
                        <div x-show="reviewOpen" x-transition @click.outside="reviewOpen = false" class="flex max-h-[90vh] w-full max-w-4xl flex-col overflow-hidden rounded-2xl border border-[var(--ui-line)] bg-[var(--ui-surface-base)] shadow-2xl">
                                    ...
                                    <tbody class="divide-y divide-[var(--ui-line)] bg-[var(--ui-surface-base)]">
                                        @forelse($previewPackages as $package)
                                            <tr>
                                                ...
                                                <td class="max-w-xs px-4 py-2"><p class="truncate text-xs font-semibold text-[var(--ui-fg-strong)]">{{ $package->transaction->payment_description ?: $package->transaction->description ?: 'Uraian belum tersedia' }}</p><p class="mt-0.5 truncate text-[11px] text-[var(--ui-fg-muted)]">{{ $package->transaction->effective_receipt_recipient_name ?: 'Penerima belum diisi' }}</p></td>
                                        ...
                                        @empty
                                            <tr><td colspan="5" class="px-5 py-8 text-center text-sm text-[var(--ui-fg-muted)]">Tidak ada paket pada triwulan ini.</td></tr>
                                        @endforelse
                                    </tbody>
                                </table>
                            </div>
