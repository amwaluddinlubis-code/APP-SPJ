        ...
        <tbody>
            @forelse($honors as $index => $honor)
                ...
                <tr>
                    ...
                    <td>{{ $transaction->no_bukti }}<br><small>{{ $transaction->spjPackage?->document_number ?: 'Belum bernomor' }}</small>
                    </td>
                    <td class="center">{{ $transaction->transaction_date?->translatedFormat('d F Y') }}</td>
                    <td>{{ $honor->name }}</td>
