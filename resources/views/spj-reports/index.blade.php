    ...
    <div class="space-y-6">
        ...
        <section class="overflow-hidden rounded-2xl border border-[var(--ui-line)] bg-[var(--ui-surface-base)] shadow" x-data="{tab: Alpine.$persist('paket')}" id="laporan-tabs">
            ...
            <div x-show="tab==='paket'" x-transition>
                <div class="border-b border-[var(--ui-line)] px-5 py-4 sm:px-6 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
                ...
                <div class="grid gap-3 p-4 lg:hidden">
                    @forelse($packages as $package)
                        <article class="rounded-xl border border-[var(--ui-line)] bg-[var(--ui-surface-base)] p-4 shadow hover:shadow transition">
                            ...
                            <div class="mt-2 flex gap-2 text-base"><span class="flex-1 rounded bg-[var(--ui-surface-soft)] px-2 py-1 text-center">Bruto {{ $rupiah($package->transaction->gross_amount) }}</span><span class="flex-1 rounded bg-amber-50 px-2 py-1 text-center text-amber-700">Pajak {{ $rupiah($package->transaction->tax_total) }}</span></div>
                        </article>
                    @empty
                ...
                <div class="hidden lg:block overflow-x-auto">
                    <table class="min-w-full divide-y divide-[var(--ui-line)] text-base" id="laporan-table">
                        ...
                        <tbody class="divide-y divide-[var(--ui-line)]" id="laporan-tbody">
                            @forelse($packages as $package)
            ...
            <div x-show="tab==='monitoring'" x-transition class="hidden" :class="tab==='monitoring' ? '!block' : ''">
                <div class="border-b border-amber-100 bg-amber-50/40 px-5 py-4 sm:px-6 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
                    ...
                    <div class="flex items-center gap-2">
                        <select onchange="const u=new URL(window.location); u.searchParams.set('pendingPerPage', this.value); u.searchParams.delete('pending_page'); window.location=u.toString()" class="rounded-lg border border-amber-200 bg-[var(--ui-surface-base)] px-2 py-1 text-base font-bold text-amber-800">
                ...
                <div class="grid gap-3 p-4 lg:hidden">
                    @forelse($pendingPaginator as $transaction)
                        <article class="rounded-xl border border-amber-200 bg-[var(--ui-surface-base)] p-4 shadow hover:shadow transition">
                            <div class="flex items-start justify-between gap-2">
                                <p class="font-mono text-base font-bold text-amber-900">{{ $transaction->no_bukti }}</p>
                                <span class="rounded-full px-2 py-0.5 text-xs font-bold {{ $transaction->spjPackage ? 'bg-amber-100 text-amber-700 border border-amber-200' : 'bg-[var(--ui-surface-muted)] text-slate-600' }}">{{ $transaction->spjPackage ? 'Draft' : 'Belum siap' }}</span>
                            </div>
                            ...
                            <div class="mt-3 text-right">
                                ...
                                @else<a href="{{ route('spj.index', ['tab' => 'persiapan', 'state' => 'unprepared']) }}" class="inline-flex rounded-lg bg-[var(--ui-surface-base)] border border-[var(--ui-line)] px-3 py-1.5 text-base font-bold hover:bg-slate-50">Buka persiapan →</a>@endif
                            </div>
                        </article>
                    @empty
                        ...
                        <tbody class="divide-y divide-amber-100" id="monitoring-tbody">
                            @forelse($pendingPaginator as $transaction)
                                <tr data-bukti="{{ $transaction->no_bukti }}" data-uraian="{{ strtolower($transaction->description ?? '') }}" data-status="{{ $transaction->spjPackage ? 'draft' : 'belum' }}" class="hover:bg-amber-50/60 transition">
                                    ...
                                    <td class="px-4 py-3"><span class="rounded-full px-2 py-0.5 text-xs font-bold {{ $transaction->spjPackage ? 'bg-amber-100 text-amber-700 border border-amber-200' : 'bg-[var(--ui-surface-muted)] text-slate-500' }}">{{ $transaction->spjPackage ? 'Draft — nomor belum ditetapkan' : 'Paket belum disiapkan' }}</span></td>
        ...
        <section class="grid gap-6 xl:grid-cols-2"><article class="overflow-hidden rounded-2xl border border-[var(--ui-line)] bg-[var(--ui-surface-base)] shadow"><div class="border-b border-[var(--ui-line)] px-5 py-4"><h2 class="font-bold text-slate-800">Realisasi per Kegiatan</h2><p class="mt-1 text-base text-slate-500">Seluruh transaksi BKU pada tahun aktif.</p></div><div class="max-h-[32rem] overflow-auto"><table class="min-w-full text-base"><thead class="sticky top-0 bg-[var(--ui-surface-soft)]"><tr><th class="px-4 py-3 text-left text-xs font-bold text-slate-500">KODE</th><th class="px-4 py-3 text-left text-xs font-bold text-slate-500">NAMA KEGIATAN</th><th class="px-4 py-3 text-right text-xs font-bold text-slate-500">REALISASI</th></tr></thead><tbody class="divide-y divide-[var(--ui-line)]">@forelse($summary['activities'] as $row)<tr><td class="px-4 py-3 font-mono text-xs text-indigo-700">{{ $row->activity_code }}</td><td class="px-4 py-3">{{ $row->activity_name }}</td><td class="px-4 py-3 text-right font-bold text-emerald-700">{{ $rupiah($row->realization) }}</td></tr>@empty<tr><td colspan="3" class="px-4 py-8 text-center text-slate-500">Belum ada realisasi.</td></tr>@endforelse</tbody></table></div></article><article class="overflow-hidden rounded-2xl border border-[var(--ui-line)] bg-[var(--ui-surface-base)] shadow"><div class="border-b border-[var(--ui-line)] px-5 py-4"><h2 class="font-bold text-slate-800">Realisasi per Rekening</h2><p class="mt-1 text-base text-slate-500">Dikelompokkan berdasarkan kode rekening transaksi.</p></div><div class="max-h-[32rem] overflow-auto"><table class="min-w-full text-base"><thead class="sticky top-0 bg-[var(--ui-surface-soft)]"><tr><th class="px-4 py-3 text-left text-xs font-bold text-slate-500">KODE</th><th class="px-4 py-3 text-left text-xs font-bold text-slate-500">NAMA REKENING</th><th class="px-4 py-3 text-right text-xs font-bold text-slate-500">REALISASI</th></tr></thead><tbody class="divide-y divide-[var(--ui-line)]">@forelse($summary['accounts'] as $row)<tr><td class="px-4 py-3 font-mono text-xs text-indigo-700">{{ $row->account_code }}</td><td class="px-4 py-3">{{ $row->account_name }}</td><td class="px-4 py-3 text-right font-bold text-emerald-700">{{ $rupiah($row->realization) }}</td></tr>@empty<tr><td colspan="3" class="px-4 py-8 text-center text-slate-500">Belum ada realisasi.</td></tr>@endforelse</tbody></table></div></article></section>
    </div>
</x-layouts.tailwind-app>
