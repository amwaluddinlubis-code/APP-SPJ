    ...
    <div class="space-y-6">
        ...
        <div class="space-y-4">
            @foreach($documentTypes as $documentType)
                ...
                <article class="overflow-hidden rounded-2xl border border-[var(--ui-line)] bg-[var(--ui-surface-base)] shadow-sm" x-data="{ open: true, pattern: @js($pattern), padding: {{ (int) $padding }}, type: @js($documentType), school: @js($school->school_code ?: $school->npsn), npsn: @js($school->npsn), year: @js((string) $year->year), month: @js(now()->format('m')), romanMonth: @js($romanMonth), tw: @js($tw), preview() { return this.pattern.replaceAll('{SEQ}', '1'.padStart(Number(this.padding), '0')).replaceAll('{TYPE}', this.type).replaceAll('{SCHOOL}', this.school).replaceAll('{NPSN}', this.npsn).replaceAll('{YEAR}', this.year).replaceAll('{MONTH}', this.month).replaceAll('{ROMAN_MONTH}', this.romanMonth).replaceAll('{TW}', this.tw) } }">
                    <button type="button" @click="open = !open" :aria-expanded="open.toString()" aria-controls="number-format-panel-{{ strtolower($documentType) }}" class="flex w-full items-center justify-between gap-3 border-b border-[var(--ui-line)] px-5 py-4 text-left">
                        <div class="min-w-0">
                            <p class="text-xs font-bold uppercase tracking-wide text-indigo-600">{{ $documentType }}</p>
                            <h2 class="mt-1 truncate font-bold text-slate-900">{{ $labels[$documentType] ?? str_replace('_', ' ', $documentType) }}</h2>
                        </div>
                        <div class="flex shrink-0 items-center gap-3">
                            <span class="w-fit rounded-full {{ $format ? 'bg-emerald-50 text-emerald-700' : 'bg-amber-50 text-amber-700' }} px-2.5 py-1 text-xs font-bold">{{ $format ? 'Tersimpan' : 'Format bawaan' }}</span>
                    ...
                    <form id="number-format-panel-{{ strtolower($documentType) }}" x-show="open" x-collapse method="POST" action="{{ route('document-number-formats.update', $documentType) }}" class="p-5 sm:p-6">@csrf @method('PUT')<input type="hidden" name="document_type" value="{{ $documentType }}">
