...
class SpjTemplateService
{
    ...
    public static function placeholderGroups(): array
    {
        return [
            ...
            'Sekolah & pejabat' => ['NAMA_SEKOLAH','NAMA_SATUAN_PENDIDIKAN','NPSN','ALAMAT_SEKOLAH','KECAMATAN','KOP_SURAT','NAMA_KEPALA_SEKOLAH','NIP_KEPALA_SEKOLAH','NAMA_BENDAHARA_BOSP','NIP_BENDAHARA_BOSP'],
            ...
            'Pesanan & pekerjaan' => ['NOMOR_PESANAN','TANGGAL_PESANAN','NOMOR_INVOICE','TANGGAL_INVOICE','STATUS_INVOICE','NOMOR_SPK','TANGGAL_SPK','TANGGAL_RAB','URAIAN_PEKERJAAN','LOKASI_PEKERJAAN','TANGGAL_MULAI','TANGGAL_SELESAI','TANGGAL_TANDA_TANGAN','TANGGAL_PENYERAHAN','TEMPAT_PENYERAHAN'],
    ...
    public function placeholders(SpjPackage $package, School $school): array
    {
        ...
        $values = [
            ...
            'TANGGAL_TRANSAKSI' => $transaction->transaction_date?->translatedFormat('d F Y') ?: '',
            ...
            'CARA_BAYAR' => (string) $transaction->payment_method,
            'REFERENSI_BAYAR' => (string) $transaction->payment_reference,
            'NOMOR_PESANAN' => (string) ($transaction->invoice_number),
            'TANGGAL_PESANAN' => $transaction->invoice_date?->translatedFormat('d F Y') ?: '',
            ...
            'NOMOR_SPK' => (string) $transaction->spk_number,
            'TANGGAL_SPK' => $transaction->spk_date?->translatedFormat('d F Y') ?: '',
            'URAIAN_PEKERJAAN' => (string) $transaction->work_description,
            'LOKASI_PEKERJAAN' => (string) $transaction->work_location,
            'TANGGAL_MULAI' => $transaction->work_started_at?->translatedFormat('d F Y') ?: '',
            'TANGGAL_SELESAI' => $transaction->work_completed_at?->translatedFormat('d F Y') ?: '',
        ...
        // Alias kompatibilitas untuk desain TPL_* Excel yang telah dipakai operator.
        // Seluruh template baru tetap disarankan memakai nama kanonis di atas.
        return $values + [
            ...
            'JENIS_SPJ' => (string) ($transaction->spj_category ?: $transaction->spj_category),
            'POTONGAN_PAJAK' => $values['TOTAL_PAJAK'],
            'PENERIMA_PENYEDIA' => $values['NAMA_PENERIMA'],
            'NAMA_PENYEDIA' => $values['NAMA_PENERIMA'],
            ...
            'TANGGAL_RAB' => $values['TANGGAL_TRANSAKSI'],
            'TANGGAL_TANDA_TANGAN' => $values['TANGGAL_TRANSAKSI'],
            'TANGGAL_PENYERAHAN' => $values['TANGGAL_SELESAI'] ?: $values['TANGGAL_TRANSAKSI'],
            'TEMPAT_PENYERAHAN' => $transaction->work_location ?: $school->address,
            // Data rekanan yang belum tersedia di sumber ARKAS dibiarkan sebagai
            // isian kosong, sehingga operator dapat melengkapinya pada template bila perlu.
            'ALAMAT_PENYEDIA' => '',
            'NPWP_PENYEDIA' => '',
            'TELEPON_PENYEDIA' => '',
            'KECAMATAN' => '',
    ...
    /**
     * Menghasilkan HTML dari template Excel yang telah diisi.
     * Ini dipakai untuk pratinjau di browser tanpa mengubah file template sumber.
     */
    ...
    /**
     * Resolve unggahan pada disk local Laravel (storage/app/private), dengan
     * fallback untuk template legacy yang tersimpan langsung di storage/app.
     */
    ...
    private function fillExcelItems(Worksheet $sheet, SpjPackage $package): void
    {
        ...
        // TPL resmi menyediakan lima baris rincian. Bila item lebih banyak,
        // perluas tabel di bawah lima baris tersebut agar total/tanda tangan turun utuh.
        ...
        // Pertahankan lima baris standar namun kosongkan slot yang tidak terpakai.
